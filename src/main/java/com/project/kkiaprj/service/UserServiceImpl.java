...
public class UserServiceImpl implements UserService {
    ...
    public int dropUser(User user) {
        int result = 0;
        User dropUser = userRepository.findById(user.getId()).orElse(null);

        if (dropUser != null) {
            // 우선 탈퇴하는 유저가 저장한 맛집 글 saveCnt, 좋아요 한 최애 글 likeCnt -1 해주기
            List<FoodSave> foodSaves = dropUser.getFoodSaves();
            for (var el : foodSaves) {
                Long foodId = el.getFood().getId();
                foodService.changeSaveCnt(-1L, foodId);
            }

            List<FavoriteLike> favoriteLikes = dropUser.getFavoriteLikes();
            for (var el : favoriteLikes) {
                Long favoriteId = el.getFavorite().getId();
                favoriteService.changeLikeCnt(-1L, favoriteId);
            }

            userRepository.delete(dropUser);
